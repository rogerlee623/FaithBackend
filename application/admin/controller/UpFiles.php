...
class UpFiles extends Common
{
    ...
    public function file(){
        ...
        $file = request()->file($fileKey['0']);
        // 移动到框架应用根目录/public/uploads/ 目录下
        $info = $file->validate(['ext' => 'zip,rar,pdf,swf,ppt,psd,ttf,txt,xls,doc,docx,mp4,mov,webm'])->move('uploads');
        if($info){
            $result['code'] = 0;
            $result['info'] = '文件上传成功!';
            $path=str_replace('\\','/',$info->getSaveName());

            $result['url'] = $this->serverUrl.'/uploads/'. $path;
    ...
    public function editUpload(){
        ...
        if($info){
            $path=str_replace('\\','/',$info->getSaveName());
            return  $this->serverUrl.'/uploads/'. $path;
        }else{
            // 上传失败获取错误信息
            $result['code'] =1;
            $result['msg'] = '图片上传失败!';
