...
class EmptyController extends Common{
    ...
    public function index(){
        if(Request::isAjax()){
            ...
            $rsult['msg'] = "获取成功";
            //增加一个管理显示
            if(session('aid')!=1 || $cinfo['is_show']!=1){
              $cinfo['is_show']=0;
            }
        ...
        foreach ( $post as $key => $val ) {
            if(isset($fields[$key])){
                ...
                if($fields[$key]['type']=='datetime'){
                    ...
                    $post[$key] =htmlspecialchars_decode($post[$fields[$key]['field']]);
                    if(isset($post['description']) && $post['description'] == '' && isset($post[$fields[$key]['field']])) {
                        $content = stripslashes($post[$fields[$key]['field']]);
